import { ArrowUpRight, Film, Globe, Headphones, ImageIcon, PlayCircle } from "lucide-react";
...
export function PortfolioHero({ onContactClick }: ContactClickProps) {
  return (
    <section className="relative overflow-hidden border-b border-white/10 pt-32 md:pt-40">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 md:grid-cols-[1.2fr_0.8fr] md:py-24">
        <div className="space-y-8">
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-purple-400">Portfolio Scaffold</p>
          ...
          <p className="max-w-2xl text-lg text-zinc-400 md:text-xl">
            This starter layout gives you a strong foundation to present creative projects with dedicated sections for motion, visuals, audio, and launched websites.
          </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            { label: "Website Links", icon: Globe },
          ].map(({ label, icon: Icon }) => (
            <div key={label} className="border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
              ...
              <p className="mt-2 text-sm text-zinc-400">Ready for your live assets and project details.</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="video-gallery" className="border-b border-white/10">
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        <SectionIntro
          ...
          title="Video gallery"
          body="Use these cards for trailers, testimonials, behind-the-scenes edits, or product explainers. Replace the placeholders with live embeds when you are ready."
        />
    ...
    <section id="image-gallery" className="border-b border-white/10 bg-zinc-950/40">
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        <SectionIntro
          eyebrow="Visual work"
          title="Image gallery"
          body="A modular image wall for stills, campaign artwork, product renders, and social creatives. Each block can later be swapped with real photos."
        />
...
export function MusicPlayerSection() {
  ...
  const progressWidth = useMemo(() => {
    ...
    return `${Math.min(85, Math.max(30, Math.round(totalSeconds / 3)))}%`;
  }, [activeTrack]);
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-[0.9fr_1.1fr] md:py-24">
        <div>
          <SectionIntro
            ...
            title="Music player"
            body="Feature original scores, sound design previews, podcast cuts, or voice samples. The interface is ready for real audio sources in the next step."
          />
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="border border-white/10 bg-zinc-950/70 p-6">
            ...
            <p className="mt-1 text-sm text-purple-300">{activeTrack.genre}</p>
            ...
            <div className="mt-8 flex items-center gap-4">
              ...
              <div className="w-full">
                <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
                  <div className="h-full rounded-full bg-purple-400" style={{ width: progressWidth }}></div>
                </div>
          ...
          <div className="border border-white/10 bg-black/30 p-3">
            {tracks.map((track) => (
              <button
                ...
                className={`flex w-full items-center justify-between border px-4 py-4 text-left transition ${activeTrack.title === track.title ? "border-purple-500 bg-purple-500/10" : "border-transparent hover:border-white/10 hover:bg-white/5"}`}
              >
                <div>
                  <p className="font-medium text-white">{track.title}</p>
                  <p className="text-sm text-zinc-400">{track.genre}</p>
                </div>
...
export function WebsiteLinksSection() {
  return (
    <section id="websites" className="bg-zinc-950/50">
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        <SectionIntro
          ...
          title="Website links"
          body="Showcase websites you have designed or developed. Update each card with the live URL, tech stack, and a short project summary."
        />
  ...
  return (
    <footer className="border-t border-white/10 bg-black/70">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-8 text-sm text-zinc-500 md:flex-row md:items-center md:justify-between">
        <p>Quantum Climb Portfolio Scaffold</p>
        <div className="flex flex-wrap gap-4">
